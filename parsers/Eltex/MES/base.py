...
class BaseEltexMESParser(BasePyParser):
    ...
    def __init__(self, managed_object):
        super(BaseEltexMESParser, self).__init__(managed_object)
    ...
    def create_parser(self):
        ...
        INTERFACE_DESCRIPTION = (Literal("description") | Literal("name")) + REST.copy().setParseAction(self.on_interface_descripion)
        ...
        # Logging
        LOGGING_HOST = LineStart() + Literal("logging") + Literal("host") + IPv4_ADDRESS.copy().setParseAction(self.on_logging_host)
        LOGGING_BLOCK = LOGGING_HOST
        # NTP
        NTP_SERVER = LineStart() + Literal("sntp") + Literal("server") + IPv4_ADDRESS.copy().setParseAction(self.on_ntp_server)
        NTP_BLOCK = NTP_SERVER
        # SNMP
        SNMP_COMMUNITY = LineStart() + Literal("snmp-server community") + (Word(alphanums + "-_") + Optional(Literal("view") + Word(alphanums + "-_")) + Optional(Literal("ro") | Literal("rw")) + Optional(DIGITS)).setParseAction(self.on_snmp_community)
